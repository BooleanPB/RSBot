...
public class Tiles extends MethodProvider {
	...
	/**
	 * Gets the tile in the middle of 2 given tiles.
	 * 
	 * @param t
	 *            First tile.
	 * @param tt
	 ...
	 */
	public RSTile getCenterBetween(RSTile t, RSTile tt) {
		if (t.getX() != -1 && t.getY() != -1 && tt.getX() != -1
				&& tt.getY() != -1) {
			return new RSTile((t.getX() + tt.getX()) / 2,
					(t.getY() + tt.getY()) / 2);
		} else {
			return null;
		}
